...
// Private functions.
	...
	static long patchKernel(unsigned long loadAddress, unsigned long cmdBase, long listSize, unsigned long textSegmentAddress, unsigned long vldSegmentAddress);
#endif
	...
	while (symbolNumber < symtab->nsyms)
	{
		...
		if ((nl->n_sect == 1 /* __TEXT,__text */) && nl->n_value)
		{
			...
			if (strcmp(symbolName, "__ZN6OSKext14loadExecutableEv") == 0)
			{
				...
				{
					//
					// New search pattern: 0x48xx74db854c3789
					//
					...
					/* Old search patterns – macOS Sierra DP1 = 89 c3 48 85 db 74 77 48 in HexEdit
					if (*(uint64_t *)p == 0x487774db8548e789)
					{ //                      ^^^^
						DEBUG_PATCH_STATUS(symbolName, ((uint64_t)p - startAddress), (symbolNumber + (skippedSymbolCount / listSize) + 1), 1)

						*(uint64_t *)p = 0x4812ebdb8548e789; // 89 c3 48 85 db eb 12 48 in HexEdit
						//                   ^^^^
						p = (unsigned char *)endAddress;
					}
			...
			if (strcmp(symbolName, "_cpuid_set_info") == 0)
			{
				...
				for (; p <= (unsigned char *)endAddress; p++)
				{
					...
					/* Broadwell_E (0x4F) -> Broadwell (0x47)
					if (*(uint64_t *)p == 0x47f883e9c083c4b6) // Reversed (0f) b6 c4 83 c0 e9 83 f8 47 (8 bytes).
					{ //                          ^^                                       ^^
						DEBUG_PATCH_STATUS(symbolName, ((uint64_t)p - startAddress), (symbolNumber + (skippedSymbolCount / listSize) + 1), 1)

						*(uint64_t *)p = 0x47f883e1c083c4b6; // Reversed (0f) b6 c4 83 c0 e1 83 f8 47 (8 bytes).
						p = (unsigned char *)endAddress;
					}
					else if (*(uint64_t *)p == 0x4ff883f1c083c4b6) // Reversed (0f) b6 c4 83 c0 f1 83 f8 4f (8 bytes).
					{ //                               ^^                                       ^^
						DEBUG_PATCH_STATUS(symbolName, ((uint64_t)p - startAddress), (symbolNumber + (skippedSymbolCount / listSize) + 1), 1)

						*(uint64_t *)p = 0x4ff883e9c083c4b6; // Reversed (0f) b6 c4 83 c0 e9 83 f8 4F (8 bytes).
						p = (unsigned char *)endAddress;
					} */
				}
			}
		...
		if (((nl->n_sect == 8 /* __DATA,__data */) && nl->n_value) && gPlatform.CPU.CstConfigMsrLocked)
		{
			...
			else if (strcmp(symbolName, "_xcpm_pkg_scope_msrs") == 0) // 7 MSRs (0x01A0, 0x01FC, 0x01AA, 0x0620, 0x064C, 0x063A, 0x0642)
			{
				...
				// New way of patching.
				//
				for (unsigned char target_msr = 0; target_msr < 7; target_msr++)
				{
					//
					// Secondary for-loop to find MSRs that we need to disable.
					//
					for (unsigned char msr_to_disable = 0; msr_to_disable < 6; msr_to_disable++)
					{
						//
